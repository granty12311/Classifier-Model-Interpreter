...
def compute_shap_values(
...
) -> Tuple[np.ndarray, shap.Explainer] | Tuple[np.ndarray, shap.Explainer, pd.DataFrame, np.ndarray]:
    """
    ...
    Parameters:
        ...
        return_sample: If True, also return the sampled X and sample indices

    Returns:
        shap_values (n_samples, n_features), explainer object
        If return_sample=True: shap_values, explainer, X_sample, sample_idx
    """
    # Sample if needed
    if sample_size and len(X) > sample_size:
        from ..utils.data_utils import sample_data
        X_sample, _, sample_idx = sample_data(X, None, sample_size, random_state, stratify=False)
        # Reset index to 0-based for consistency with SHAP values array
        X_sample = X_sample.reset_index(drop=True)
    else:
        X_sample = X.reset_index(drop=True) if return_sample else X
